    ...
    if (!errors.length) {
      ...
      if (!errors.length) {
        if (!models.templateContent.mandatory(obj.template)) {
          ...
          if (validateSize) {
            const validSize = await models.templateContent.size(obj.template.content, obj.template.encodingType, limit);
            if (!validSize) {
              errors.push({
...
module.exports = {
  init(options) {
    maxFileSize = options && (options.maxFileSize || options['max-file-size']) || process.env.FILE_UPLOADS_MAX_FILE_SIZE || '25MB';
    try {
