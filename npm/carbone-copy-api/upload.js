...
module.exports = {
  ...
  init(options) {
    const fileUploadsDir = (options && options.fileUploadsDir) || process.env.FILE_UPLOADS_DIR || fs.realpathSync(os.tmpdir());
    try {
    ...
    const formFieldName = (options && options.formFieldName) || process.env.FILE_UPLOADS_FORM_FIELD || 'files';
